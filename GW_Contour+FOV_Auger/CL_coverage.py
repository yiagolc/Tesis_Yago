...
if "CL_Coverage" not in Scripts.split():
        ...
        # MJD-OBS y OBJECT y la asigna a time_index y object_index, respectivamente
         ...
         if "Limit_Flux" in Scripts:
             ...
             process = subprocess.run([ 'python3', 'Limit_Flux.py','no', FILENAME, Scripts ])
...
if GCN_mode == "yes":
    ...
    print('Executing Limit_Flux.py for', GWname)
    Flux_limit = subprocess.run([ 'python3', 'Limit_Flux.py', 'yes', FILENAME], capture_output=True, text=True)
    ...
    if len(Scripts.split()) == 4: # If we want all, we send something like a Circular
